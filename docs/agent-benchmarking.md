...
Completed configurations are keyed and retained, so profile reruns resume instead of repeating successful work. The agent also records runtime parameter sets that the installed llama.cpp build reports as unsupported, then skips them in later runs instead of repeatedly failing. Models marked **CORRUPT** are not retried. The current TinyDolphin GGUF is recorded as corrupt because llama.cpp reported tensor data outside the file bounds. The agent refuses new sweeps when memory is too constrained and persists every completed result before returning a recommendation.
